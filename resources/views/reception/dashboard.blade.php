...
@section('page-title', __('Welcome, Receptionist'))
@section('page-subtitle', __('Reception Dashboard'))
...
<div class="mb-8 animate-slide-up">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <a href="{{ route('reception.register-patient') }}" class="dashboard-card bg-white rounded-xl shadow p-6 flex items-center space-x-4 hover:shadow-lg">
            ...
            <div>
                <h3 class="font-bold text-gray-800">{{ __('New Patient') }}</h3>
                <p class="text-gray-600 text-sm">{{ __('Register a new patient') }}</p>
            </div>
        </a>
        ...
        <a href="{{ route('reception.schedule-appointments') }}" class="dashboard-card bg-white rounded-xl shadow p-6 flex items-center space-x-4 hover:shadow-lg">
            ...
            <div>
                <h3 class="font-bold text-gray-800">{{ __('Book Appointment') }}</h3>
                <p class="text-gray-600 text-sm">{{ __('Schedule new appointment') }}</p>
            </div>
        </a>
        ...
        <a href="{{ route('reception.manage-walkin') }}" class="dashboard-card bg-white rounded-xl shadow p-6 flex items-center space-x-4 hover:shadow-lg">
            ...
            <div>
                <h3 class="font-bold text-gray-800">{{ __('Check Walk-ins') }}</h3>
                <p class="text-gray-600 text-sm">{{ __('Manage walk-in patients') }}</p>
            </div>
        </a>
    </div>
</div>
...
<div class="mb-8 animate-slide-up" style="animation-delay: 0.1s">
    <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-bold text-gray-800">{{ __('Today\'s Appointments') }}</h2>
        <a href="{{ route('reception.schedule-appointments') }}" class="text-ddu-primary hover:underline font-medium">{{ __('View All') }}</a>
    </div>
...
<div class="animate-slide-up" style="animation-delay: 0.2s">
    <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-bold text-gray-800">{{ __('Recent Registrations') }}</h2>
        <a href="{{ route('reception.search-patients') }}" class="text-ddu-primary hover:underline font-medium">{{ __('View All') }}</a>
    </div>
